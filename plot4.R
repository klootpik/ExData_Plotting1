...
### Now it is finally time to make some plots
...
# then make ready the plot for 4 subplots
par(mfcol=c(2, 2))

# first one
with(Databoy, plot(x = Supertime, y = Global_active_power, ylab = "Global Active Power", xlab = "", type = "l"))

# second one
with(Databoy, plot(x = Supertime, y = Sub_metering_1, ylab = "Energy sub metering", xlab = "", type = "n"))

with(Databoy, points(x = Supertime, y = Sub_metering_1, type = "l"))
with(Databoy, points(x = Supertime, y = Sub_metering_2, type = "l", col = "red"))
with(Databoy, points(x = Supertime, y = Sub_metering_3, type = "l", col = "blue"))

legend("topright", legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty = 1, col = c("black", "red", "blue"), bty = "n")

# third one
with(Databoy, plot(x = Supertime, y = Voltage, xlab = "datetime", ylab = "Voltage", type = "l"))

# fourth and final one
with(Databoy, plot(x = Supertime, y = Global_reactive_power, xlab = "datetime", ylab = "Global_reactive_power", type = "l"))

dev.copy(png, file = "plot4.png", width = 480, height = 480)
dev.off()

# And now it is time to go outside for a run!